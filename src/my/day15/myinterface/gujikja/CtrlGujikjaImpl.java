...
public class CtrlGujikjaImpl implements CtrlGujikja {
	...
	*/
	@Override
	public void register(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
		
		if(CommonMemberAbstract.count < cmbr_arr.length) { // 지금까지 생성된 구직자 회원수가 cmbr_arr.length(==> 정원) 보다 적을 경우에만 신규회원을 받아들인다.
		
			GujikjaImpl gu = new GujikjaImpl();
			...
			gu.setType(1);
			cmbr_arr[CommonMemberAbstract.count++] = gu;
			System.out.println("\n>> 구직자 회원 가입 성공했습니다.<<\n");
		} 
	...
	// === 구직자 로그인 === //
	@Override
	public GujikjaImpl login(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
	...
	// === 구직자 전용메뉴 ===
	@Override
	public void gu_menu(Scanner sc, GujikjaImpl login_gu, CommonMemberAbstract[] cmbr_arr, RecruitImpl[] rc_arr, RecruitApplyImpl[] rcApply_arr) {
	...
	// === 내정보 보기 ===
	private void view_myInfo(GujikjaImpl login_gu) {
	/*
	   >>> 엄정화님의 정보 <<<
	   ---------------------------------------------------------------------------------
    ...
    */
	...
	// === 내정보 수정 === 
	private void update_myInfo(Scanner sc, GujikjaImpl login_gu) {
	...
	// === 모든구인회사 조회 === 
	private void view_all_company_info(CommonMemberAbstract[] cmbr_arr) {
	...
	// === 구인회사검색하기 ===
	private void search_company(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
	...
	// === 업종검색 === //
	private void search_jobtype_company(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
		...
		for(int i = 0; i< CommonMemberAbstract.count; i++) {
			if(cmbr_arr[i].getType()==2 &&
				((CompanyImpl)cmbr_arr[i]).getJob_type().toLowerCase().contains(job_type_name)) {
				is_existence = true;
				sb.append(cmbr_arr[i].getInfo()+"\n");
			}
		}// end of for-------------------------
	...
	// === 자본금검색 ===	
    private void search_seedmoney_company(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
    	...
    	try {
    		...
    		for(int i = 0; i< CommonMemberAbstract.count; i++) {
    			if(cmbr_arr[i].getType()==2 && 
    				((CompanyImpl)cmbr_arr[i]).getSeed_money() >= search_seed_money) {
    				is_existence = true;
    				sb.append(cmbr_arr[i].getInfo()+"\n");
    			}
    		}// end of for-------------------------
    ...
    // === 모든채용공고조회(현재 채용이 진행중인 것만 조회) ===
 	private void view_all_recruit_info(RecruitImpl[] rc_arr) {
    ...
    // === 채용응모하기 ===
  	private void input_recruitApply(Scanner sc, GujikjaImpl login_gu, RecruitImpl[] rc_arr, RecruitApplyImpl[] rcApply_arr) {
