...
public class CtrlCompanyImpl implements CtrlCompany {
	...
	*/
	...
	public void register(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
		...
		if(CommonMemberAbstract.count < cmbr_arr.length) { // 지금까지 생성된 구인회사 회원수가 cmbr_arr.length(==> 정원) 보다 적을 경우에만 신규회원을 받아들인다.
		
			CompanyImpl cp = new CompanyImpl();
			...
			cp.setType(2);
			cmbr_arr[CommonMemberAbstract.count++] = cp;
	...
	// === 구인회사 로그인 ===
	@Override
	public CompanyImpl login(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
	...
	// === 구인회사 전용메뉴 ===
	@Override
	public void cp_menu(Scanner sc, CompanyImpl login_cp, CommonMemberAbstract[] cmbr_arr, RecruitImpl[] rc_arr, RecruitApplyImpl[] rcApply_arr) {
    ...
    private void search_ageline_gujikja(Scanner sc, CommonMemberAbstract[] cmbr_arr) {
		...
		if(!is_exists_gujikja) {
			...
			for(int i = 0; i< CommonMemberAbstract.count; i++) {
				try {
					if(cmbr_arr[i].getType()==1 
					   && MyUtil.getAge(((GujikjaImpl)cmbr_arr[i]).getJubun())/10*10 == Integer.parseInt(input_ageline)
					  ) {
						  isFind = true;
						  sb.append(cmbr_arr[i].getInfo() + "\n");
					    }
	 		...
	 		for(int i = 0; i< RecruitApplyImpl.count; i++) {
	 			...
	 			if( login_cp.getId().equals(rcApply_arr[i].getRc().getCp().getId()) ) {
	 				...
	 				sb.append(rcApply_arr[i].getRc().getRecruit_no()+"\t"+
	 						  ...
	 						  MyUtil.getAge(rcApply_arr[i].getGu().getJubun())+"\t"+
