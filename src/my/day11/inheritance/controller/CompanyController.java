...
public class CompanyController {
	...
	private List<RecruitPost> createDefaultCompany(List<Company> comp) {
		RecruitPost rp1 = RecruitPost.fromBuilder().company(comp.get(0)).title("아무나 다뽑습니다.").workType("정규직").cnt(5)
				.salGrade(3000).finishDay("20250427").build();
		RecruitPost rp2 = RecruitPost.fromBuilder().company(comp.get(1)).title("대기업 들어올 사람").workType("정규직").cnt(3)
				.salGrade(5000).finishDay("20250522").build();
		RecruitPost rp3 = RecruitPost.fromBuilder().company(comp.get(2)).title("채용공고 제목이야").workType("계약직").cnt(10)
				.salGrade(2000).finishDay("20250827").build();
		RecruitPost rp4 = RecruitPost.fromBuilder().company(comp.get(2)).title("마감기한 지났져").workType("계약직").cnt(10)
				.salGrade(2000).finishDay("20240827").build();
		RecruitPost rp5 = RecruitPost.fromBuilder().company(comp.get(0)).title("아무나 다뽑습니다.길게작성해보기").workType("정규직")
				.cnt(5).salGrade(3000).finishDay("20250727").build();
		return List.of(rp1, rp2, rp3, rp4, rp5);
	}

	private List<Company> createDefaultCompany() {
		Company comp1 = Company.builder().withId("comp1").withName("서브마스").withPassword("12341234a!")
				.businessNum("1200520005").seedMoney(1000000000L).jobType("서비스업").build();
		Company comp2 = Company.builder().withId("comp2").withName("켄싱턴호텔").withPassword("12341234a!")
				.businessNum("1345526005").seedMoney(6000000000L).jobType("서비스업").build();

		Company comp3 = Company.builder().withId("comp3").withName("빕스").withPassword("12341234a!")
				.businessNum("1230531005").seedMoney(700000000L).jobType("서비스업").build();
	...
	public void searchCompany(Scanner sc) {
		while (true) {
			...
			switch (select) {
			case "1":
				searchCompanyByJobType(sc);
				break;
			case "2":
				searchCompanyByName(sc);
				break;
			case "3":
				searchCompanyById(sc);
				break;
			case "4":
				searchCompanyBySeedMoney(sc);
				break;
			case "5":
				return;
			default:
				System.out.println("잘못 입력됨 1 부터 5 중에 입력 해주세요.");
			}
		}
	...
	private void searchCompanyBySeedMoney(Scanner sc) {
		...
		if (!resultList.isEmpty())
			printCompanyInfo(resultList);
		else
			System.out.println("[" + new DecimalFormat("#,###").format(seedMoney) + "원] 이상의 자본금을 보유한 회사가 없습니다.");
	}
	...
	private void searchCompanyByName(Scanner sc) {
		...
		if (!resultList.isEmpty())
			...
			System.out.println("회사명에 [" + input + "] 이 포함된 회사가 없습니다.");
	}
	...
	private void searchCompanyByJobType(Scanner sc) {
		...
		if (!resultList.isEmpty())
			...
			System.out.println("[" + jobType + "] 업종의 회사가 없습니다.");
	}
	...
	public void loginLogic(Scanner sc, Company loginUser, JobSeekerController jsc) {
		...
		main: while (true) {
			...
			case "7":// 우리회사 채용공고 지원자 조회
				...
				jsc.searchMyCompanyJoinUser(rp, sc);
				break;
			case "8":
				break main; // 로그아웃

			default:
				System.out.println("잘못 입력됨 1 부터 10 중 입력하세요.");
			}
		}
...
//	}
	...
	public void signUp(Scanner sc) {
		...
		companyList.add(Company.builder().withId(id).withPassword(password).businessNum(businessNum).withName(name)
				.seedMoney(Integer.parseInt(seedStr)).build());
	...
	private void modifyMyInfo(Company loginUser, Scanner sc) {
		...
		Company originalUser = Company.builder().withId(loginUser.getId()).withName(loginUser.getName())
				.withPassword(loginUser.getPassword()).businessNum(loginUser.getBusinessNum())
				.jobType(loginUser.getJobType()).seedMoney(loginUser.getSeedMoney()).build();
		...
		main: while (true) {
			...
			case "6":
				...
				System.out.println("자본금 수정 성공 : " + new DecimalFormat("#,###").format(beforeMoney) + "원 에서 "
						+ loginUser.parsingSeedMoney() + "원으로 수정하였습니다.");
				break;
			case "7":
			...
			case "8":
				...
				break main;
			default:
				System.out.println("잘못입력됨 1 부터 8 중에 입력해주세요.");
			}
		}
	}
	...
	public void printAllPostInfo(Scanner sc) {
		...
		if (!isBefore)
			System.out.println("채용 마감 기한이 지난 게시물을 제외한 게시물을 조회합니다.");
		else
			System.out.println("모든 게시물을 조회합니다.");

		List<RecruitPost> validList = isBefore ? this.postList : getValidPost();
		if (validList.isEmpty()) {
			System.out.println("진행중인 채용 공고가 없습니다.");
			return;
		}

		System.out.println("=".repeat(40) + " 채용 공고 조회 결과 " + "=".repeat(40));
		System.out.println("조회된 게시물 : " + validList.size() + "개"
				+ (!isBefore ? "　　　　　기간이 지난 게시물 : " + (postList.size() - validList.size()) + "개" : ""));

		printPostInfo(validList);
	}

	private List<RecruitPost> getValidPost() {
		return postList.stream().filter(p -> LocalDate.parse(p.getFinishDay()).plusDays(1).isAfter(LocalDate.now()))
				.toList();
	}
	...
	private boolean existUserId(String id) {
		return companyList.stream().filter(js -> js.getId().equals(id)).findFirst().isPresent();
	}
	...
	private void searchCompanyById(Scanner sc) {
		...
		if (result.isPresent())
			printCompanyInfo(List.of(result.get()));
		else
			System.out.println("구인회사 아이디 [" + input + "] 로 검색된 회사가 없습니다.");
	}

	private Optional<Company> findCompanyById(String id) {
		return companyList.stream().filter(js -> js.getId().equals(id)).findFirst();
	}
	...
	private void createRecruitPost(Scanner sc, Company loginUser) {
		...
		RecruitPost newPost = RecruitPost.fromBuilder().company(loginUser).title(title).workType(workType).cnt(cnt)
				.salGrade(salGrade).finishDay(finishDay).build();
	...
	public Optional<RecruitPost> findPostById(int postId) {
		return postList.stream().filter(p -> p.getPostId() == postId).findAny();
	}
	...
	private String getFinishDayRepeat(Scanner sc) {
		while (true) {
			...
			if (input.length() == 8 && input.matches("^[2][0-9]{7}$")) {
				...
				try {
					LocalDate finishDay = LocalDate.of(Integer.parseInt(year), Integer.parseInt(month),
							Integer.parseInt(day));
					if (finishDay.isAfter(LocalDate.now()))
						return input;
	...
	public RequestJoined userJoinCompany(Scanner sc, JobSeeker loginUser,List<RequestJoined> allJoin) {
		int postId = getPostIdRepeat(sc);
		
		RecruitPost post = findPostById(postId)
				.orElseThrow(() -> new RuntimeException("[" + postId + "] 번호의 채용 공고가 존재하지 않습니다."));
		
		List<RequestJoined> filterJoin = allJoin.stream().filter(j->j.getRecruitPost().equals(post)).toList();
		
		if(filterJoin.size() >= post.getCnt())
			throw new RuntimeException("지원가능한 인원이 초과되어 지원이 불가합니다.");
		...
		//선택한 게시물에 이미 지원했는지 확인
		boolean existJoin = filterJoin.stream().anyMatch(j->j.getJobSeeker().equals(loginUser));
		if(existJoin) throw new RuntimeException("이미 지원한 채용 공고입니다.");
		while (true) {
			System.out.print("지원 동기를 입력해주세요.\n=>");
			String motive = sc.nextLine().strip();
			if (!motive.isBlank())
	...
	public void printPostMission() {
		
		printEndGuide();
		System.out.println("공고번호\t회사명\t채용제목\t근무형태\t채용인원\t연봉\t마감일자");
		
		postList.stream().filter(post -> LocalDate.parse(post.getFinishDay()).isAfter(LocalDate.now()))
				.forEach(post -> 
				System.out.println(post.getPostId() + "\t" + post.getCompany().getName() + "\t" + post.getTitle() + "\t"
								+ post.getWorkType() + "\t" + post.getSalGrade() + "\t" + post.getFinishDay()));
		printEndGuide();
